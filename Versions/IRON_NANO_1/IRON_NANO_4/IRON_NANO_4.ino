...
#define SIZE_BAR (9 * 5)
#define TIME_20MINS             0.1*60//       20*60
#define TIME_30MINS             0.1*60//       30*60
//=======================================================
...
int secs, mins = 0;
int current20Minutes, current30Minutes;
int previous20Minutes, previous30Minutes = 0;
boolean tim_20mins, tim_30mins = false;
//=======================================================
...
void setup() 
{
  ...
  Timer1.initialize(1000000);//Timer1.initialize(150000); // set a timer of length 150000 microseconds (or 0.15 sec)
  Timer1.attachInterrupt(timer1_ISR); // attach the service routine here  

  pinMode(9, OUTPUT);
  digitalWrite(9,HIGH);  
  
  pinMode(IND_20MIN_LED, OUTPUT);
  digitalWrite(IND_20MIN_LED, LOW);
...
{  
  ...
  if (digitalRead(STANDBYin) == HIGH)
  {
    ...
    { 
      ...
      lcd.setCursor(6,1);
      lcd.print("      ");
      current20Minutes = mins;
      current30Minutes = mins;
      if(current20Minutes - previous20Minutes > TIME_20MINS)
      {
        previous20Minutes = current20Minutes; 
        tim_20mins = true;
        tim_30mins = false;
        digitalWrite(IND_20MIN_LED, HIGH);
      }  
      if(current30Minutes - previous30Minutes > TIME_30MINS)
      {
        previous30Minutes = current30Minutes; 
        tim_20mins = true;
        tim_30mins = true; 
        digitalWrite(IND_20MIN_LED, LOW);
        pwm = 0;
  ...
  if((tim_20mins == true) && (tim_30mins == true)) 
  {
    pwm = 0;
    digitalWrite(PWMpin, LOW);
  }  
