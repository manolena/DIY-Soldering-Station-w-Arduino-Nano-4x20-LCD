...
volatile float fast = 1.0;
volatile float slow = 0.1;
//=======================================================
...
void setup() 
{
  ...
  Serial.println("D:\\MANOS\\DXP\\AVR SOLDERING IRON\\FIRMWARE\\NANO\\IRON_NANO_8");
  ...
  Timer1.initialize(500000); // set a timer of length 150000 microseconds (or 0.15 sec)
...
void checkTimer()
{
  ...
  if(mins >= TIMER_20MIN)
  {
    ...
    lcd.setCursor(0,2);
    lcd.print("  WILL GO TO SLEEP  ");
    delay(5000);
    lcd.setBacklight(LOW);
    while(digitalRead(STANDBYin) != HIGH)
    {
...
void checkUnplugged()
{
  if((actual_temperature >= MAX_TEMP_C) && (actual_temperature < MAX_TEMP_C+50))
  {    
    pwm = 0;
    digitalWrite(HEAT_LED, LOW);
...
void showTemps()
{
  ...
  if(celsiusOrFahrenheit == true) 
  {
    print_histogram(t1,MAX_TEMP_C); 
    lcd.print("  ");
  }  
  else if(celsiusOrFahrenheit == false) 
  {
    print_histogram(t1,MAX_TEMP_F); 
    lcd.print("  ");
  }  
  lcd.setCursor(2,1);
  if(celsiusOrFahrenheit == true) 
  {
    print_histogram(t2,MAX_TEMP_C); 
    lcd.print("  ");
  }  
  else if(celsiusOrFahrenheit == false) 
  {
    print_histogram(t2,MAX_TEMP_F); 
    lcd.print("  ");
  }  
  showTime(13,0);
}   
...
{
  myEncoder.lowLevelTick(fast, slow);
